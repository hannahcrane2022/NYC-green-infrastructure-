...
green_infrastructure <- read_csv ("GI_assets_publicMap_construct (1).csv") %>%
  clean_names() %>%
  select (asset_type, asset_id, status, borough, dep_contra, street_add, city_counc, tree_commo, status_gro)
...
all_boroughs <- ddply(GI_selected, .(status), nrow) %>%
  adorn_totals("row") %>%
...
test <- left_join (Staten_Island, projects_SI, by ="V1")
rm(test)

Bronx <- subset(green_infrastructure, borough == 'Bronx')
Bronx <- ddply(Bronx1, .(status), nrow) %>%
  adorn_totals("row")%>%
...
Manhattan <- subset(green_infrastructure, borough == 'Manhattan') %>%
