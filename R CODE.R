...
projects_top <- ddply(green_infrastructure, .(asset_type), nrow) %>%
  filter(V1 > 86)

projects_bottom <- ddply(green_infrastructure, .(asset_type), nrow) %>%
  filter(V1 < 5)
...
74/195
low_complaint <- complaint_gi_nta %>%
  filter(Complaint_Count < 100 , GI_Count > 100 )
...
write_csv(projects_bottom, "10 Least Common.csv")

write_csv(projects_top, "10 Most Common.csv")
